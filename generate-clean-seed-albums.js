// Script to generate cleaned seed-albums.ts with only albums that exist in Supabase
// Run: node generate-clean-seed-albums.js
...
`;
  ...
  // Write to file
  fs.writeFileSync('seed-albums-cleaned.ts', output);
  ...
  console.log(`Output file: seed-albums-cleaned.ts`);
  console.log('\nGenre breakdown:');
  
  sortedGenres.forEach(genre => {
    console.log(`  ${genre.padEnd(30)} ${byGenre[genre].length} albums`);
  });
  
  console.log('\nReplace /lib/musicbrainz/seed-albums.ts with seed-albums-cleaned.ts');
}
